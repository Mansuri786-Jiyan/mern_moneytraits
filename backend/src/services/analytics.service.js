...
import { differenceInDays, subYears, subDays } from "date-fns";
...
export const chartAnalyticsService = async (userId, dateRangePreset, customFrom, customTo) => {
    const range = getDateRange(dateRangePreset, customFrom, customTo);
    const { from, to, value: rangeValue } = range;
    const filter = {
        userId: new mongoose.Types.ObjectId(userId),
...
export const expensePieChartBreakdownService = async (userId, dateRangePreset, customFrom, customTo) => {
    const range = getDateRange(dateRangePreset, customFrom, customTo);
    const { from, to, value: rangeValue } = range;
    const filter = {
        userId: new mongoose.Types.ObjectId(userId),
