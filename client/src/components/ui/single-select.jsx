...
const CommandEmpty = forwardRef(({ className, ...props }, forwardedRef) => {
    ...
    if (!render)
        return null;
    return (_jsx("div", { ref: forwardedRef, className: cn("py-6 text-center text-sm", className), "cmdk-empty": "", role: "presentation", ...props }));
});
CommandEmpty.displayName = "CommandEmpty";
const SingleSelector = React.forwardRef(({ value, onChange, placeholder, defaultOptions = [], options: arrayOptions, delay, onSearch, onSearchSync, loadingIndicator, emptyIndicator, disabled, groupBy, className, selectFirstItem = true, creatable = false, triggerSearchOnFocus = false, commandProps, inputProps, }, ref) => {
    ...
    const CreatableItem = () => {
        ...
        Object.values(options).forEach((optGroup) => {
            if (optGroup.some((opt) => opt.value === inputValue || opt.label === inputValue)) {
                exists = true;
            }
        });
        if (exists ||
        ...
        const Item = (_jsx(CommandItem, { value: inputValue, className: "cursor-pointer", onMouseDown: (e) => {
                e.preventDefault();
                e.stopPropagation();
            }, onSelect: (value) => {
    ...
    return (_jsxs(Command, { ref: dropdownRef, ...commandProps, value: commandValue, className: cn("h-auto overflow-visible bg-transparent", commandProps?.className), shouldFilter: false, filter: commandFilter(), children: [_jsxs("div", { className: cn("flex items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2", className), onClick: () => {
                    if (disabled)
                        return;
                    ...
                    if (triggerSearchOnFocus) {
                ...
                }, children: [selected ? (_jsxs("div", { className: "flex flex-1 items-center", children: [selected.label, !disabled && (_jsx("button", { type: "button", className: "ml-2", onClick: (e) => {
                                    e.stopPropagation();
                                    handleUnselect();
                                }, children: _jsx(X, { className: "h-4 w-4" }) }))] })) : (_jsx(CommandPrimitive.Input, { ...inputProps, ref: inputRef, value: inputValue, disabled: disabled, onValueChange: (value) => {
                            setInputValue(value);
                        ...
                        }, placeholder: placeholder, className: cn("flex-1 bg-transparent outline-none placeholder:text-muted-foreground", inputProps?.className) })), _jsx(ChevronsUpDown, { className: "h-4 w-4 opacity-50" })] }), _jsx("div", { className: "relative", children: open && (_jsx(CommandList, { className: "absolute top-1 z-10 w-full rounded-md border bg-popover text-popover-foreground shadow-md outline-none animate-in", onMouseLeave: () => {
                        setOnScrollbar(false);
                    }, onMouseEnter: () => {
                        setOnScrollbar(true);
                    }, onMouseUp: () => {
                        inputRef?.current?.focus();
                    }, children: isLoading ? (_jsx(_Fragment, { children: loadingIndicator })) : (_jsxs(_Fragment, { children: [EmptyItem(), CreatableItem(), !selectFirstItem && (_jsx(CommandItem, { value: "-", className: "hidden" })), Object.entries(options).map(([key, dropdowns]) => (_jsx(CommandGroup, { heading: key, className: "h-full overflow-auto", children: _jsx(_Fragment, { children: dropdowns
                                        .filter((option) => showAllOptions ||
                                        option.label
                                            .toLowerCase()
                                            .includes(inputValue.toLowerCase()))
                                        .map((option) => {
                                        return (_jsx(CommandItem, { value: option.label, disabled: option.disable, onMouseDown: (e) => {
                                                e.preventDefault();
                                                e.stopPropagation();
                                            }, onSelect: () => {
